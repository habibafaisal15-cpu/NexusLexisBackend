...
CREATE INDEX IF NOT EXISTS idx_lawyers_marketplace_search ON lawyer_profiles (city, verification_stat, membership_tier DESC) WHERE is_suspended IS FALSE;
...
CREATE INDEX IF NOT EXISTS idx_client_activities_recent ON client_activities (client_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_orders_active_pipeline ON service_orders (status) WHERE status IN ('processing', 'in_progress');
...
CREATE INDEX IF NOT EXISTS idx_appointments_slots_lookup ON appointments (lawyer_prof_id, appointment_date, appointment_time);
...
CREATE INDEX IF NOT EXISTS idx_ca_appointments_lookup ON ca_appointments (ca_prof_id, appointment_date, appointment_time);
...
CREATE INDEX IF NOT EXISTS idx_reviews_lawyer_approved ON lawyer_reviews (lawyer_id, created_at DESC) WHERE is_approved IS TRUE;
...
CREATE INDEX IF NOT EXISTS idx_messages_thread_stream ON messages (thread_id, created_at DESC) INCLUDE (is_read);
...
SELECT * FROM auth_users ORDER BY created_at DESC LIMIT 5;
SELECT * FROM users ORDER BY date_joined DESC LIMIT 5;
SELECT * FROM signup_otps ORDER BY created_at DESC LIMIT 5;

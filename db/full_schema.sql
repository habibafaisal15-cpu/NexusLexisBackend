...
-- Run: psql -U postgres -d nexuslexis -f backend/db/full_schema.sql
-- ============================================================================
...
ALTER TABLE service_orders ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP;

ALTER TABLE appointments ADD COLUMN IF NOT EXISTS client_notes TEXT;
ALTER TABLE notifications ADD COLUMN IF NOT EXISTS audience VARCHAR(20) DEFAULT 'client';

-- OTP signup functions (used by auth_backend/db/otpRepository.js)
